...
for( i in 1:len) {
 ...
 if( eff[i]>0 & length(df$Y_test_label[singleton])>n_min) {
  ...
  roc1<-roc(df$Y_test_label[singleton], y_fitted, ci=TRUE, plot = FALSE,quiet =TRUE)
...
for(i in 1:len){
 ...
 if( length(unique( df$Y_test_label[prs>=alpha1 | prs<alpha2]))>1 ) {
  ...
  y_fitted <- predict(mod_full, type="fitted")
  roc2<-roc(df$Y_test_label[prs>=alpha1 | prs<alpha2], y_fitted, ci=TRUE, plot = FALSE,quiet =TRUE)
